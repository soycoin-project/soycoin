...
#if defined(NDEBUG)
# error "Soycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x20cd69e2533ff723f92f0fa22dbc8aef0775067153d0d1a57b5193e826eb4832");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Soycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Soycoin Signed Message:\n";
...
{
    // Soycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 150500 blocks, which will occur approximately every 261 days
    nSubsidy >>= (nHeight / 150500); // Soycoin: 150500 blocks in ~261 days

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Soycoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Soycoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Soycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Soycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x743ea63999e93e5a547acfb8488855739246cecfb9ff688aaaee60b70fb00376");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Soy is cool";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0463675c37c5bee75645576b15a0a1c708245ea0dfea550340d217346894b7f0b0b35761ef383377d73cda2ff0c7837c22650380ca275efd5bbc67ebb530c23b87") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1594353865;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085127589;
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc7, 0xb4, 0xdl }; // Soycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SoycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SoycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SoycoinMiner(CWallet *pwallet)
{
    printf("SoycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("soycoin-miner");
    ...
    try { loop {
        ...
        printf("Running SoycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SoycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SoycoinMiner, pwallet));
}
